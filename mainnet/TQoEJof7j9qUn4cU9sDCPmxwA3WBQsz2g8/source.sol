...
contract TronBank is Ownable {
    using SafeMath for uint256;
    uint256 public constant DEVELOPER_RATE = 12; //per thousand
    uint256 public constant MARKETING_RATE = 48;
    ...
    constructor() public {
        refundActivated = true;
        developerAccount_ = 0x16059610D6fA40c9E8C840651adF796F7dd9ec73;  // TByeZR1rZfs6cEfaxDG2yWSNjZ4Tr1tHyZ
        marketingAccount_ = 0x6e5f8CFa238AF17C2192a01eAa7b3085890B33f2;  // TL2oo91BbszSXES4LJDzTcmtvGT1NF2szJ
        _init();
    }
    ...
    function deactivateRefund() public onlyOwner {
        refundActivated = false;
    }

    function refund() public onlyOwner {
        require(refundActivated, "refund is permanently deactivated");
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 developerRefAmount;
        uint256 marketingRefAmount;
        (developerRefAmount, marketingRefAmount) = _calculateReferrerReward(uid, _amount, investor.referrer);

        totalInvestments_ = totalInvestments_.add(_amount);

        uint256 developerPctAmount = (_amount.mul(DEVELOPER_RATE)).div(1000);
        developerAccount_.transfer(developerPctAmount.add(developerRefAmount));
        uint256 marketingPctAmount = (_amount.mul(MARKETING_RATE)).div(1000);
        marketingAccount_.transfer(marketingPctAmount.add(marketingRefAmount));
        return true;
    }
    ...
    function _calculateReferrerReward(uint256 _uid, uint256 _investment, uint256 _referrerCode) private returns (uint256, uint256) {
        uint256 _allReferrerAmount = (_investment.mul(REFERENCE_RATE)).div(1000);
        uint256 _developerAmount = 0;
        uint256 _marketingAmount = 0;
        ...
        if (_allReferrerAmount > 0) {
            uint256 _totalRate = DEVELOPER_RATE + MARKETING_RATE;
            _developerAmount = (_allReferrerAmount.mul(DEVELOPER_RATE)).div(_totalRate);
            _marketingAmount = (_allReferrerAmount.mul(MARKETING_RATE)).div(_totalRate);
        }

        return (_developerAmount, _marketingAmount);
    }
}
