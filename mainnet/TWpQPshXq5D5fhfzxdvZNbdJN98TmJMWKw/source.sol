...
contract TronHEX{
    ...
    uint256 tokenPriceTimestamp = 1552414405;
    ...
    function sellHEXTokens(uint256 amountToSell) public {
        ...
        uint256 amountOfTRXToReceive = SafeMath.div(SafeMath.mul(getTokenPrice(), amountToSell), 1000000);
        ...
        if(msg.sender != dev){
            affiliateCommision[dev] = SafeMath.add(affiliateCommision[dev], fee);
        }

        msg.sender.transfer(amountOfTRXToReceive);
    }
    ...
    function getTokenPrice() public view returns(uint256) {
        ...
        return SafeMath.add(500000, SafeMath.div(SafeMath.mul(SafeMath.div(SafeMath.mul(2, 1000000), 1000), secondsPassed), 86400));
    }
    /* EXCHANGE END */
    ...
    function stake(uint256 HEXtoStake, address referral) public {
        ...
        uint256 commision = SafeMath.div(SafeMath.mul(HEXtoStake, 6), 100);
        if(referral != msg.sender && referral != 0x1 && activeInvestment[referral] >= 2000000000){
            tokenBalance[referral] = SafeMath.add(tokenBalance[referral], commision);
        }
        if(msg.sender != dev){
            tokenBalance[dev] = SafeMath.add(tokenBalance[dev], commision);
        }
    ...
    function unstake() public {
        ...
        releaseAmount = SafeMath.sub(releaseAmount, SafeMath.div(SafeMath.mul(15, releaseAmount), 100));
    ...
    /*
    * Only dev can call this function, we automated it through our backend however.
    * It sets the daily variation rate.
    * The daily variaiton rate is an addition to the base rate (3.50% a day) which CANNOT be changed.
