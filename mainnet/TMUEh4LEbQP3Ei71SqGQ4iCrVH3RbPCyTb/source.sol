 ...
 * 95% of what is Deposited is in account.  This keeps people from getting self referral advantage and withdrawing.
 * 
...
contract TronPaysTRX {
    ...
    uint256 public minimum = 10000000;
    ...
    function buy(address _customerAddress, address _referredBy) public payable {
       ...
       uint256 deposit = (msg.value.mul(95).div(100));
       totaldeposited = totaldeposited.add(deposit);
       deposits[_customerAddress] = deposits[_customerAddress].add(deposit);
       ...
       emit DepositTRX(_customerAddress, _referredBy, deposit);
    }
    
    /**
    * @dev getAllStats
    * @param _address Address of depositor
    * getBalance - Account Current Withdrawals without referrals
    * checkReferral - Account Current Referrals
    * checkWithdrawals - Account Current Withdrawals
    * checkdeposits - Account Deposits
    * checkTotalWithdrawal - Account Total Withdrawals
    * minutesSinceListWithdrawDeposit - minutes since last 2% withdrawal
    */
    function allUserStats(address _address) public view returns(uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 balance = getBalance(_address);
        uint256 minutesSince = minutesSinceListWithdrawDeposit(_address);
        uint256 refer = referrer[_address];
        uint256 withdrawal = withdrawals[_address];
        uint256 dep = deposits[_address];
        uint256 tot = totwithdraws[_address];
        return (balance, refer, withdrawal, dep, tot, minutesSince);
    }

    function allContractStats() public view returns(uint256, uint256, uint256, uint256, uint256) {
        return (address(this).balance, users, totaldeposited, totalwithdrawn, totaldonated);
    }
