...
contract TronHeist2 {
    ...
    uint256 internal base_time_per_key = 15;
    uint256 internal bonus_potential_key_time = 30;
    ...
    Percent.percent private m_investorsPercent = Percent.percent(50, 100); // dividend split
    ...
    Percent.percent private m_instantwinPercent = Percent.percent(5, 100); 
    ...
    //uint[3] public topKeys; //can be any size
    mapping (uint => NumOfKeys[]) dayTopKeys; // day => numOfKeys // USED

    //mapping (uint => DayBuysTotals) dayBuysTotals; // (day => totalKeys)
    ...
    uint public soft_deadline_duration = 24 hours; // // max soft deadline
    bool public gamePaused = false;
    bool public limitedReferralsMode = true; 
    uint public MAX_JACKPOT_ROLLOVERS = 10; 


    uint public instantWinChance = 600; 
    uint public safetyDepositChance = 200; 
    ...
    uint public totalInstantWinsWon;
    uint public totalSafetyDepositsWon;
   ...
   // (hex) 41d00af37ad4accdd29c08ab17777047f1a3b7f50f
    rando internal rando_I = rando(0xd00af37ad4accdd29c08ab17777047f1a3b7f50f);
    ...
    function isRoundOnDark() internal view returns (bool) {
        if(rounds[latestRoundID].softDeadline - now < m_DarkMinutes && rounds[latestRoundID].softDeadline - now > 0 seconds) {  
            return true;
        } else {
            return false;
        }
    ...
    function reinvestReturns(uint value, uint _instantWinCode, uint _safetyDepositCode) public  {        
        reinvestReturns(value, address(0x0), _instantWinCode, _safetyDepositCode);
    }

    function reinvestReturns(uint value, address ref, uint _instantWinCode, uint _safetyDepositCode) public  {        
        GameRound storage rnd = rounds[latestRoundID];
    ...
    function finalizeAndRestart(address _referer, uint _instantWinCode, uint _safetyDepositCode) public payable {
        finalizeLastRound();
        startNewRound(_referer, _instantWinCode, _safetyDepositCode);
    }
    
    
    function startNewRound(address _referer, uint _instantWinCode, uint _safetyDepositCode) public payable checkDayRollover {
    ...
    function lastFreeKeyTime(address _player) public view returns (uint _lastFreeKeyTime) {
        return lastDailyFreeKeyTime[_player];
    }
    function timeToFreeKey(address _player) public view returns(uint _timeToFreeKey) {
        return now - lastDailyFreeKeyTime[_player];
    }
    
    function dailyFreeKey() public notOnPause checkDayRollover {
        ...
        if(rounds.length > 0) {
            ...
            _purchase(rnd, minInvestment, address(0x0), instantWinChance, safetyDepositChance, false); // false = no funds added to the pot
        } else {
            revert("Not yet started");
        }
    }

    function buyKeys(address _referer, uint _instantWinCode, uint _safetyDepositCode) public payable notOnPause checkDayRollover {
    ...
    function _splitRevenue(GameRound storage rnd, uint value, address ref) internal {
        ...
        uint safetyDeposit_value = m_airdropPercent.mul(value);
        uint instantwin_value = m_instantwinPercent.mul(value);
        ...
        rnd.safetyDepositPot = rnd.safetyDepositPot.add(safetyDeposit_value);
        instantWinJackpot = instantWinJackpot.add(instantwin_value);
    ...
    function _safetyDepositPrize(GameRound storage rnd, uint value, uint _safetyDepositCode) internal returns (uint _SafetyDepositAmount, uint _safetyDepositResult) {
        ...
        if(value > safetyDepositMinSun) { // 1000 TRX - TOTEST
            ...
            //if(chance % airdropChance == 0) {// once in 200 chance
    ...
    function _instantWin(uint _instantWinCode) internal returns (uint _instantWinAmount, uint _instantWinResult) {
        ...
        if(chance == _instantWinCode) {// once in 600 chance
            uint prize = instantWinJackpot;
            instantWinJackpot = 0;
            vaults[msg.sender].totalReturns = vaults[msg.sender].totalReturns.add(prize);
            unclaimedReturns = unclaimedReturns.add(prize);
            totalInstantWinsWon += prize;
