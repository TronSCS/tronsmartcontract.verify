...
contract tronGold is Ownable {
    ...
    uint256 public term = 25*60*60*24;
    ...
    function calculate(address _addr) internal {
        ...
        if(!investor.isExpired) {
            uint256 expireTime = investor.investTime.add(term);
            uint256 minutePassed = 0;
            if(now < expireTime) {
                minutePassed = (now.sub(investor.time)).div(PERIOD);
            } else {
                minutePassed = (expireTime.sub(investor.time)).div(PERIOD);
                investor.isExpired = true;
            }
    ...
    function distributeRef(uint256 _amount, address _referrer) private{
        ...
        uint256 _allRef = (_amount.mul(11)).div(100);
