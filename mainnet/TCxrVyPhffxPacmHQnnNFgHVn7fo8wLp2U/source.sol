...
contract Place is ERC721Token, Ownable {
  mapping(uint256 => string) internal contentURIs;
  mapping(uint256 => string) internal names;
  address internal creator;

  string internal rootUri="https://tronvr.space/uri/";
  uint256 public tokenPrice=1000*1e6;
  constructor () public
    ERC721Token('TronVR place', 'TVR')
  { 
    creator = msg.sender;
  }
    function setRootUri(string newRootUri) public onlyOwner{
        rootUri=newRootUri;
    }
  ...
  function mint(address _to, uint256 _tokenId, string _name) public onlyOwner {
    super._mint(_to, _tokenId);
  ...
  function tokenURI(uint256 _tokenId) public view returns (string) {
    return (String.appendUintToString(rootUri, _tokenId));
  }
...
// Set a name of this place. 
  function setName(uint256 _tokenId, string _name) public {
    require(exists(_tokenId));
    require(ownerOf(_tokenId) == msg.sender);
    names[_tokenId] = _name;
  }
...
// Get the name of this place
  ...
  // Set a URL to load this build data from. 
  ...
  function getPlaceInfo(uint256 _tokenId) public view returns(string,address,string){
      string storage name=names[_tokenId];
      string storage content=contentURIs[_tokenId];
      address owner = ownerOf(_tokenId);
        return (name,owner,content);
  }
