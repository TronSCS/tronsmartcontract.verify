...
contract Tronsilver is TRC20 {
    
    string public constant name  = "TronsILVER";
    ...
    // Constructor
    function Tronsilver(address schemaAddress) public payable {
        owner = msg.sender;
    ...
    function buyTronUnit(uint256 unitId, uint256 amount) external payable SnapshotCheck {
        ...
        uint256 devFund = SafeMath.div(SafeMath.mul(tronCost, 15), 100); // 15% dev fee
    ...
    function buyUpgrade(uint256 upgradeId) external payable SnapshotCheck {
        ...
        if (tronCost > 0) {
            ...
            uint256 devFund = SafeMath.div(SafeMath.mul(tronCost, 15), 100); // 15% for dev
