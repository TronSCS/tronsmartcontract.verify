...
contract TBPro is Managable {
    ...
    uint256 public constant TNT_INVEST_DAILY_BASE_RATE = 10;
    ...
    address public constant TNT_CONTRACT = address(0x414637fa7544be818e43b2c78206df407577c3b1e2);
    bool private supportTNT_ = false;
    uint256 public tntRate = 10;
    ...
    event onTNTInvest(address indexed investor, uint256 amount);
    ...
    function setTntRate(uint256 _value) public onlyOwner {
        tntRate = _value;
    }
    ...
    function setDividensAccount(address _newDeveloperAccount) public onlyOwner {
        require(_newDeveloperAccount != address(0));
        dividendsAccount_ = _newDeveloperAccount;
    }

    function getDividensAccount() public view onlyAdmin returns (address) {
    ...
    function _init() private {
        ...
        //reinvest 60 days
        investmentPlans_.push(Objects.Plan(2, 10, 0, true));
        //TNT, unlimited
    }
            ...
            if (investor.plans[i].isExpired) {
                ...
                if (investmentPlans_[investor.plans[i].planId].term > 0) {
                    if (block.timestamp >= investor.plans[i].investmentDate.add(investmentPlans_[investor.plans[i].planId].term)) {
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, investor.plans[i].planId, investor.plans[i].investmentDate.add(investmentPlans_[investor.plans[i].planId].term), investor.plans[i].lastWithdrawalDate);
                    } else {
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, investor.plans[i].planId, block.timestamp, investor.plans[i].lastWithdrawalDate);
                    }
                } else {
                    newDividends[i] = _calculateDividends(investor.plans[i].investment, investor.plans[i].planId, block.timestamp, investor.plans[i].lastWithdrawalDate);
                }
    ...
    function tntInvest(uint256 _amount) public payable {
        require(isSupportTNT(), "doesn't support tnt investment");
        if (!TNT_CONTRACT.call(bytes4(bytes32(keccak256("burn(uint256)"))), _amount)) {
            revert();
        }

        if (_invest(msg.sender, 2, 0, _amount * tntRate)) {
            emit onTNTInvest(msg.sender, msg.value
    ...
    function _withdraw() private isNotLocked returns (uint256) {
        ...
        for (uint256 i = 0; i < uid2Investor[uid].planCount; i++) {
            ...
            uint256 amount = _calculateDividends(uid2Investor[uid].plans[i].investment, uid2Investor[uid].plans[i].planId, withdrawalDate, uid2Investor[uid].plans[i].lastWithdrawalDate);
    ...
    function _calculateDividends(uint256 _amount, uint256 _planId, uint256 _now, uint256 _start) private pure returns (uint256) {
        uint256 dif = (_now - _start);
        uint256 div = 0;
        if (_planId == 0) {
            if (dif > 60 * DAY) {
                dif = 60 * DAY;
            ...
            div = _amount *
            (TNT_INVEST_DAILY_BASE_RATE * dif) / DAY
            / 1000;
        } else {
            //do nothing
        }
