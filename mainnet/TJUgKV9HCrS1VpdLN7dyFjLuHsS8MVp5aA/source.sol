...
contract BankOfTRX is Ownable {
    ...
    uint256 public constant MINIMUM = 10000000; 
    uint256 public constant REFERRER_CODE = 666; 
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(40, 0, true, 0)); 
        investmentPlans_.push(Objects.Plan(50, 45*60*60*24, true, 0)); 
        investmentPlans_.push(Objects.Plan(60, 25*60*60*24, true, 0)); 
        investmentPlans_.push(Objects.Plan(70, 18*60*60*24, true, 0)); 
    }
    ...
    function getCurrentPlans() public view returns (uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory, uint256[] memory) {
        ...
        bool[] memory enables = new bool[](investmentPlans_.length);
        uint256[] memory expiredTimes = new uint256[](investmentPlans_.length);
        for (uint256 i = 0; i < investmentPlans_.length; i++) {
            ids[i] = i;
            enables[i] = investmentPlans_[i].isEnabled;
            if (!investmentPlans_[i].isEnabled) {
                continue;
            }
            if (investmentPlans_[i].expiredTime > 0 && now >= investmentPlans_[i].expiredTime) {
                continue;
            }
            interests[i] = investmentPlans_[i].dailyInterest;
            terms[i] = investmentPlans_[i].term;
            expiredTimes[i] = investmentPlans_[i].expiredTime;
        }
        return
        (
        ids,
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        require(_planId >= 0 && _planId < investmentPlans_.length && investmentPlans_[_planId].isEnabled && now > investmentPlans_[_planId].expiredTime, "Wrong investment plan id");
