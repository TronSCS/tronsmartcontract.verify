...
contract TronMax is Ownable {
    using SafeMath for uint256;
    uint256 public constant VAULT_RATE = 100; 
    ...
    uint256 public totalReferrals_;
    uint256 public totalDonations_;
    uint256 public totalGrants_;
    uint256 public totalIntegrations_;
    uint256 public totalWithdraws_;
    uint256 public totalTxs_;
    ...
    event onDonation(address donor, uint256 amount);
    event onIntegration(address integrator, address investor,uint256 amount);
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(38, 0)); //unlimited
        investmentPlans_.push(Objects.Plan(48, 45*60*60*24)); //45 days
        investmentPlans_.push(Objects.Plan(58, 25*60*60*24)); //25 days
        investmentPlans_.push(Objects.Plan(68, 18*60*60*24)); //18 days
    }
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        totalTxs_ = totalTxs_.add(1);

        uint256 vaultPercentage = (_amount.mul(VAULT_RATE)).div(1000);
        vaultAccount_.transfer(vaultPercentage);
    ...
    function grant(address addr, uint256 _planId) public payable {
        ...
        if (_invest(addr,_planId,referrerCode,msg.value)) {
            totalGrants_ = totalGrants_.add(msg.value);
    ...
    function donate() public payable {
        if(msg.value > 0)
        {
            totalDonations_ = totalDonations_.add(msg.value);
            totalTxs_ = totalTxs_.add(1);
            emit onDonation(msg.sender, msg.value);
        }
    }

    function integrate(address addr, uint256 _planId) public payable {
        uint256 integratorUid = address2UID[msg.sender];
        bool isAutoAddReferrer = true;
        uint256 referrerCode = 0;

        if (integratorUid != 0 && isAutoAddReferrer) {
            referrerCode = integratorUid;
        }

        if (_invest(addr,_planId,referrerCode,msg.value)) {
            totalIntegrations_ = totalIntegrations_.add(msg.value);
            emit onIntegration(msg.sender, addr, msg.value);
        }
    }
    ...
    function withdraw() public payable {
        ...
        totalWithdraws_ = totalWithdraws_.add(withdrawalAmount);
        totalTxs_ = totalTxs_.add(1);
    ...
    function _calculateReferrerReward(uint256 _uid, uint256 _investment, uint256 _referrerCode) private {
        ...
        if (_referrerCode != 0) {
            ...
            if (_ref1 != 0) {
                ...
                totalReferrals_ = totalReferrals_.add(_refAmount);
        ...
        if (_allReferrerAmount > 0) {
            vaultAccount_.transfer(_allReferrerAmount);
        }
    }
