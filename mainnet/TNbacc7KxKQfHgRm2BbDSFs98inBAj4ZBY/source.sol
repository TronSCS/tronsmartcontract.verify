...
contract ChestMining is Random, AccessService {
    ...
    uint256 public prizePoolPercent = 80;
    ...
    function adjustPoolPercent(uint256 newPoolPercent) external onlyAdmin whenNotPaused {
        ...
        prizePoolPercent = newPoolPercent;
    }
    ...
    function miningOneRaceCoin(uint256 amount)
        external
        whenNotPaused
    {
        require(amount > 0);
        raceCoinContract.chestRaceCoinCost(amount, msg.sender);

        uint256 seed = _rand();
        uint16[13] memory attrs = _getFashionParam(seed);

        tokenContract.createFashion(msg.sender, attrs, 4);
    ...
    function miningOneSelf() 
        ...
        whenNotPaused
    {
        require(msg.value >= 63000000);

        uint256 seed = _rand();
        uint16[13] memory attrs = _getFashionParam(seed);

        tokenContract.createFashion(msg.sender, attrs, 2);
        _transferHelper(63000000);

        if (msg.value > 63000000) {
            msg.sender.transfer(msg.value - 63000000);
        }
        ...
        whenNotPaused
    {
        ...
        _transferHelper(189000000);

        if (msg.value > 189000000) {
            msg.sender.transfer(msg.value - 189000000);
        }
        ...
        whenNotPaused
    {
        ...
        _transferHelper(300000000);

        if (msg.value > 300000000) {
            msg.sender.transfer(msg.value - 300000000);
        }
        ...
        whenNotPaused
    {
        ...
        _transferHelper(567000000);

        if (msg.value > 567000000) {
            msg.sender.transfer(msg.value - 567000000);
        }
