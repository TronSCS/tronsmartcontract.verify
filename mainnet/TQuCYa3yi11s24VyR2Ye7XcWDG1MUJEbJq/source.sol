...
contract GeneMixerInterface {
    ...
    function mixGenes(uint40 momId, uint256 genes1, uint40 dadId, uint256 genes2) external view returns (uint256);

    function canBreed(uint40 momId, uint256 genes1, uint40 dadId, uint256 genes2) external view returns (bool);
}
...
contract BlockchainCutiesCore /*is ERC721, CutieCoreInterface*/
{
    ...
    event BirthWithGeneration(address indexed owner, uint40 cutieId, uint40 momId, uint40 dadId, uint256 genes, uint16 generation, uint16 cooldownIndex, uint40 birthTime);
    ...
    event CooldownIndexChanged(uint40 indexed cutieId, uint16 oldValue, uint16 newValue);
    ...
    /// @dev The address of utils function, like tokenURI construction or signature recover.
    ...
    function getCooldownIndexFromGeneration(uint16 _generation, uint40 _cutieId) internal view returns (uint16)
    {
        return config.getCooldownIndexFromGeneration(_generation, _cutieId);
    }
    ...
    returns (uint40)
    {
        ...
        emit BirthWithGeneration(_owner, newCutieId, _cutie.momId, _cutie.dadId, _cutie.genes, _cutie.generation, _cutie.cooldownIndex, _cutie.birthTime);
    ...
    modifier onlyOperator() {
        require(operatorAddress[msg.sender] || msg.sender == ownerAddress);
        _;
    }
    ...
    /// @dev Called from plugin contract when using items as effect
    function changeGenes(uint40 _cutieId, uint256 _genes) public whenNotPaused onlyPlugin
    {
    ...
    function changeOptional(uint40 _cutieId, uint64 _optional) public whenNotPaused onlyPlugin
    {
        Cutie storage cutie = cuties[_cutieId];
...
*/
    ...
    uint40 public promoLimit = 50000;
    ...
    function createGen0AuctionWithTokens(uint256 _genes, uint128 startPrice, uint128 endPrice, uint40 duration, address[] allowedTokens) public onlyOperator
    {
        require(gen0CutieCreatedCount < gen0Limit);
        uint40 cutieId = _createCutie(0, 0, 0, 0, _genes, address(this), uint40(now));
        _approve(cutieId, saleMarket);

        saleMarket.createAuctionWithTokens(
            cutieId,
            startPrice,
            endPrice,
            duration,
    ...
    function createPromoCutieWithGeneration(uint256 _genes, address _owner, uint16 _generation) public onlyOperator
    {
        require(promoCutieCreatedCount < promoLimit);
        promoCutieCreatedCount++;
        gen0CutieCreatedCount++;

        uint16 cooldownIndex = config.getCooldownIndexFromGeneration(_generation, 0);

        _createCutie(0, 0, _generation, cooldownIndex, _genes, _owner, uint40(now));
    }

    function createPromoCutieBulk(uint256[] _genes, address _owner, uint16 _generation) public onlyOperator
    {
        require(promoCutieCreatedCount + _genes.length <= promoLimit);
        promoCutieCreatedCount += uint40(_genes.length);
        gen0CutieCreatedCount += uint40(_genes.length);

        uint16 cooldownIndex = config.getCooldownIndexFromGeneration(_generation, 0);

        for (uint i = 0; i < _genes.length; i++)
        {
            _createCutie(0, 0, _generation, cooldownIndex, _genes[i], _owner, uint40(now));
        }
    }
    ...
    function setBreedingMarketAddress(address _breedingAddress) public onlyOwner
    {
    ...
    function setSaleMarketAddress(address _saleAddress) public onlyOwner
    {
    ...
    ///  Does some ownership trickery for creating auctions in one transaction.
    ///  Optional money amount can be sent to contract to feature auction.
    function createSaleAuctionWithTokens(
        uint40 _cutieId,
        uint128 _startPrice,
        uint128 _endPrice,
        uint40 _duration,
        ...
        payable
    {
        ...
        // Sale auction throws if inputs are invalid and clears
        // transfer and sire approval after escrowing the cutie.
        saleMarket.createAuctionWithTokens.value(msg.value)(
            _cutieId,
            _startPrice,
            _endPrice,
            _duration,
    ...
    function _triggerCooldown(uint40 _cutieId, Cutie storage _cutie) internal
    {
        ...
        _cutie.cooldownEndTime = config.getCooldownEndTimeFromIndex(_cutie.cooldownIndex, _cutieId);
    ...
    function _breedWith(uint40 _momId, uint40 _dadId) internal returns (uint40)
    {
        ...
        uint16 babyGen = config.getBabyGenFromId(_momId, _dadId);

        // Call the gene mixing operation.
        uint256 childGenes = geneMixer.mixGenes(_momId, mom.genes, _dadId, dad.genes);

        // Make the new cutie
        address owner = cutieIndexToOwner[_momId];
        uint40 cutieId = _createCutie(_momId, _dadId, babyGen, getCooldownIndexFromGeneration(babyGen, 0), childGenes, owner, mom.cooldownEndTime);
        ...
        returns (uint)
    {
        ...
        uint256 childGenes = geneMixer.mixGenes(0, 0x0, _dadId, dad.genes);

        // tutorial pet id is zero
        uint40 cutieId = _createCutie(0, _dadId, babyGen, getCooldownIndexFromGeneration(babyGen, 0), childGenes, msg.sender, 12);
    ...
    function withdrawBalances() external onlyOperator
    {
    ...
    function withdrawEthFromBalance(uint256 limit) external
    {
        require(
    ...
    /// @dev Sets the reference to the breeding auction.
    /// @param _utilsAddress - Address of utils contract.
    function setUtilsAddress(UtilsInterface _utilsAddress) external onlyOwner
    {
        utils = _utilsAddress;
    }
    ...
    function tokenURI(uint256 _tokenId) external view returns (string infoUrl)
    {
        return utils.tokenURI(_tokenId);
    }

    function approveAndCall(address _spender, uint _tokenId, bytes data) external returns (bool success) {
    ...
    function transferMany(address to, uint[] tokens) public
    {
    ...
    modifier onlyPlugin() {
        require(plugins.isPlugin(msg.sender));
        _;
    }


    function setPlugins(address _address) public onlyOwner {
        plugins = PluginsInterface(_address);
    }

    function checkOwnerAndApprove(address _claimant, uint40 _cutieId, address _pluginsContract) external
    {
        require(msg.sender == address(plugins));
        require(_isOwner(_claimant, _cutieId));
        _approve(_cutieId, _pluginsContract);
    }
}
