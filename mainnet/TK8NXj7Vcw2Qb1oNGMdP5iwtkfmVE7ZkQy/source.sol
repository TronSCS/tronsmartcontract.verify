...
contract BttBank is Ownable {
    ...
    address private adminAccount_;
    address private tntAccount_;
    ...
    constructor() public {
        ...
        adminAccount_ = msg.sender;
        tntAccount_ = msg.sender;
        _init();
    }

    modifier onlyAdmin() {
        require(msg.sender == owner || msg.sender == adminAccount_);
        _;
    }
    ...
    function setSupportTNT(bool _value) public onlyOwner {
        supportTNT = _value;
    }

    function isSupportTNT() public view onlyAdmin returns (bool) {
        return supportTNT;
    }

    function setAdminAccount(address _newAdminAccount) public onlyOwner {
        require(_newAdminAccount != address(0));
        adminAccount_ = _newAdminAccount;
    }

    function getAdminAccount() public view onlyAdmin returns (address) {
        return adminAccount_;
    }

    function setTNTAccount(address _newTNTAccount) public onlyOwner {
        require(_newTNTAccount != address(0));
        tntAccount_ = _newTNTAccount;
    }

    function getTNTAccount() public view onlyAdmin returns (address) {
        return tntAccount_;
    }
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(0, 36, 0, true));
        //unlimited
        investmentPlans_.push(Objects.Plan(1, 46, 45 * 60 * 60 * 24, true));
        //45 days
        investmentPlans_.push(Objects.Plan(2, 56, 25 * 60 * 60 * 24, true));
        //25 days
        investmentPlans_.push(Objects.Plan(3, 66, 18 * 60 * 60 * 24, true));
        //18 days
    }

    function addInvestmentPlan(uint256 _id, uint256 _dailyInterestRate, uint256 _term, bool _active) public onlyOwner {
        if (_id < investmentPlans_.length) {//existing plan
            investmentPlans_[_id].dailyInterest = _dailyInterestRate;
            investmentPlans_[_id].term = _term;
            investmentPlans_[_id].isActive = _active;
        } else {
            investmentPlans_.push(Objects.Plan(_id, _dailyInterestRate, _term, _active));
        }
    }

    function getCurrentPlans() public view returns (uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory) {
    ...
    function getBalance() public view returns (uint256) {
        return address(this).tokenBalance(BTT_ID);
    }
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        if (supportTNT) {
            uint256 tntPercentage = (_amount.mul(TNT_RATE)).div(1000);
            tntAccount_.transferToken(tntPercentage, BTT_ID);
        } else {
            uint256 developerPercentage = (_amount.mul(DEVELOPER_RATE)).div(1000);
            developerAccount_.transferToken(developerPercentage, BTT_ID);
        }

        uint256 marketingPercentage = (_amount.mul(MARKETING_RATE)).div(1000);
        marketingAccount_.transferToken(marketingPercentage, BTT_ID);
        return true;
    }

    function grant(address _addr, uint256 _planId) public payable {
        ...
        if (_invest(_addr, _planId, referrerCode, msg.tokenvalue)) {
            emit onGrant(msg.sender, _addr, msg.tokenvalue);
        }
    }

    function invest(uint256 _referrerCode, uint256 _planId) public payable {
        require(msg.value == 0, "wrong trx amount");
        if (_invest(msg.sender, _planId, _referrerCode, msg.tokenvalue)) {
    ...
    function withdraw() public payable {
        ...
        if (withdrawalAmount >= 0) {
            msg.sender.transferToken(withdrawalAmount, BTT_ID);
    ...
    function reinvest(uint256 _planId) public payable {
        uint256 withdrawalAmount = _withdraw();
        if (withdrawalAmount >= 0) {
            if (_invest(msg.sender, _planId, 0, withdrawalAmount)) {//existing user, _referrerCode is useless, just pass 0
    ...
    function _calculateReferrerReward(uint256 _uid, uint256 _investment, uint256 _referrerCode) private {
        ...
        if (_allReferrerAmount > 0) {
            referenceAccount_.transferToken(_allReferrerAmount, BTT_ID);
        }
    }
