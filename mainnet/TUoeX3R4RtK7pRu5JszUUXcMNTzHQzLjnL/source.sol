...
contract CarsPresell is AccessService, ICarsPresell {
    ...
    ///Bonus pool ratio
    uint256 public prizeGoldPercent = 80;
    ...
    function CarsPresell(address _nftAddr) public {
		...
		carCountsLimit = 10000;

        carPresellCounter[10001] = 500;
        carPresellCounter[10002] = 500;
        carPresellCounter[10003] = 500;
        carPresellCounter[10004] = 500;
        carPresellCounter[10005] = 500;
		carPresellCounter[10006] = 500;
    ...
    function adjustPoolPercent(uint256 newPoolPercent) external onlyAdmin whenNotPaused {
        require(newPoolPercent > 0 && newPoolPercent <= 100);   
        prizeGoldPercent = newPoolPercent;
    }
        ...
        whenNotPaused 
    {
        ...
        if (_equipmentId == 10001) {
            require(msg.value >= 408000000 );
            payBack = (msg.value - 408000000 );
            ...
            require(msg.value >= 610000000 );
            payBack = (msg.value - 610000000 );
            ...
            require(msg.value >= 1226000000 );
            payBack = (msg.value - 1226000000 );
            ...
            require(msg.value >= 3068000000 );
            payBack = (msg.value - 3068000000 );
            ...
            require(msg.value >= 9265000000 );
            payBack = (msg.value - 9265000000 );
            ...
            require(msg.value >= 32700000000 );
            payBack = (msg.value - 32700000000 );
        ...
        if (referer != address(0) && referer != msg.sender) {
            ...
            PresellReferalGain(referer, msg.sender, referalDivs);
        }
    ...
    function carSellByRaceCoin( address player,uint16 _equipmentId) 
        external
        whenNotPaused 
    {
        require(actionContracts[msg.sender]);
        ...
        uint16 curSupply = carPresellCounter[_equipmentId];
        require(curSupply > 0);
        uint16[] storage buyArray = presellLimit[player];
        uint256 curBuyCnt = buyArray.length;
	
        uint256 payBack = 0;
        if (_equipmentId == 10001) {
            uint16[13] memory param1 = [10001, 1, 9, 10, 0, 0, 0, 5, 0, 0, 0, 0, 0];       // 10 productivity 5% productivity plus
            tokenContract.createFashion(player, param1, 1);
            raceCoinContract.increasePlayersAttribute(player, param1);
            buyArray.push(10001);   
        } else if(_equipmentId == 10002) {
            uint16[13] memory param2 = [10002, 2, 9, 15, 0, 0, 0, 8, 5, 0, 0, 0, 0];       // 15 productivity 8% productivity plus 5% attack bonus
            tokenContract.createFashion(player, param2, 1);
            raceCoinContract.increasePlayersAttribute(player, param2);
            buyArray.push(10002);
        } else if(_equipmentId == 10003) {
            uint16[13] memory param3 = [10003, 3, 9, 30, 0, 0, 0, 15, 10, 5, 0, 0, 0];        // 30 productivity 15% productivity plus 10% attack plus 5% defense plus
            tokenContract.createFashion(player, param3, 1);
            raceCoinContract.increasePlayersAttribute(player, param3);
            buyArray.push(10003);
        } else if(_equipmentId == 10004) {
            uint16[13] memory param4 = [10004, 4, 9, 75, 0, 0, 0, 38, 25, 13, 5, 0, 0];        // 75 productivity 38% productivity plus 25% attack plus 13% defense plus 5% predatory addition.
            tokenContract.createFashion(player, param4, 1);
            raceCoinContract.increasePlayersAttribute(player, param4);
            buyArray.push(10004); 
        } else if(_equipmentId == 10005){
            uint16[13] memory param5 = [10005, 5, 9, 225, 0, 0, 0, 113, 75, 38, 15, 0, 0];      // 225 productivity 113% productivity plus 75% attack plus 38% defense plus 15% predatory addition.
            tokenContract.createFashion(player, param5, 1);
            raceCoinContract.increasePlayersAttribute(player, param5);
            buyArray.push(10005);
        }else if(_equipmentId == 10006){
            uint16[13] memory param6 = [10006, 6, 9, 788, 0, 0, 0, 394, 263, 131, 53, 0, 0];      // 788 productivity 394% productivity plus 263% attack plus 131% defense plus 53% predatory addition.
            tokenContract.createFashion(player, param6, 1);
            raceCoinContract.increasePlayersAttribute(player, param6);
            buyArray.push(10006);
        }
    ...
    function carsAirDropByAdmin( address player,uint16 _equipmentId) external onlyAdmin whenNotPaused 
    {
        require(_equipmentId >= 10001 && _equipmentId <= 10006);
        uint16 curSupply = carPresellCounter[_equipmentId];
        require(curSupply > 0);
        uint16[] storage buyArray = presellLimit[player];
        uint256 curBuyCnt = buyArray.length;
