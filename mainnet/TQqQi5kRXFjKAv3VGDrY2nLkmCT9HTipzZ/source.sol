...
contract T1T{
    ...
    string public name = "TronOne Token";
    string public symbol = "T1T";
    uint8 constant public decimals = 18;
    uint8 constant internal entryFee_ = 10;
    uint8 constant internal transferFee_ = 1;
    uint8 constant internal exitFee_ = 1;
    uint8 constant internal t1Rate_ = 33;
    ...
    address public _t1; 
     ...
     constructor(address vault, address t1)
        public
    {
        _vault = vault;
        _t1 = t1;
    ...
    function sell(uint256 _amountOfTokens) onlyBagholders public {
        ...
        uint256 _t1Bonus = SafeMath.div(SafeMath.mul(_dividends,t1Rate_),100);
        if(address(this).balance >= _t1Bonus)
        {
            address(_t1).transfer(_t1Bonus);
        }

        _dividends = SafeMath.sub(_dividends, _vaultBonus);
        _dividends = SafeMath.sub(_dividends, _t1Bonus);
    ...
    function purchaseTokens(uint256 _incomingTron) internal returns (uint256) {
        ...
        uint256 _t1Bonus = SafeMath.div(SafeMath.mul(_undividedDividends,t1Rate_),100);
        _dividends = SafeMath.sub(_dividends, _t1Bonus);
        if(address(this).balance >= _t1Bonus)
        {
