...
/***
 * https://faith-of-tron.com/BabyWhales.html
 * marketing: TWEdWArzDbDef51S6KDYfK2imFcGu8NnjA
 */
...
contract BabyWhales {
    ...
    /// @dev P3D pricing
    uint256 constant internal tokenPriceInitial_ = 10; //0.00001 trx
    uint256 constant internal tokenPriceIncremental_ = 1; //0.000001trx
    ...
    uint256 public maxEarlyStake = 150000000000; //150000trx
    uint256 public whaleBalanceLimit = 1000000000000; //1000000trx

    /// @dev light the marketing
    address public marketing;
	
    /// @dev starting
    uint256 public startTime = 1554397200; //  04/04/2019 @ 5:00pm (UTC)
   ...
   constructor (address _marketing) public {
    ...
    function buySPD(address _referredBy) antiEarlyWhale isControlled public payable  returns (uint256) {
    ...
    function purchaseFor(address _referredBy, address _customerAddress) antiEarlyWhale isControlled public payable returns (uint256) {
    ...
    function entryFee() public view returns (uint8){
     ...
     // @dev Function about jackpot info
	...
	// @dev Function about start time
	function startInfo() public view returns (uint256 timer, uint256 timer2) {
		if (startTime > now) {
			timer = startTime - now;
			timer2 = 0;
		} else {
			timer = 0;
			timer2 = now - startTime;
		}
	}
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy , address _customerAddress) internal returns (uint256) {
		...
		uint256 _marketing = SafeMath.div(SafeMath.mul(_incomingTron, 25), 1000); //2.5%
     ...
     * The last investor (with 500+ TRX) will receive the jackpot in 3 minutes
     */	
	function calculateJackPot(uint256 _incomingTron, address _customerAddress) internal {
		uint256 timer = SafeMath.div(SafeMath.sub(now, jackPotStartTime_), 60 minutes); 
		if (timer > 0 && jackPotPretender_ != address (0) && jackPot_ > 0) {
			...
			jackPotPretender_.send(jackPot_);
			jackPot_ = 0;
			jackPotStartTime_ = now;
		...
		if (_incomingTron >= 1000000000) { //1000trx
