...
contract Tron666 is Ownable {
    ...
    uint256 public constant timeDivider= 60*60*24; // 60*60*24 Production;  60 Test
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(200, 0)); //unlimited
    }
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256 referrerEarnings, uint256 availableReferrerEarnings, uint256 referrer, uint256 referrer_rate,uint256 level1RefCount, uint256 level2RefCount, uint256 level3RefCount, uint256 planCount,uint256[] memory currentDividends, uint256[] memory newDividends) {
        if (msg.sender != owner) {
        ...
        referrer_rate= investor.referrer_rate;
    ...
    function _addInvestor(address _addr, uint256 _referrerCode) private returns (uint256) {
        ...
        uid2Investor[latestReferrerCode].referrer_rate = 50;
        if (_referrerCode >= REFERRER_CODE) {
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 referrer_rate = _calculateReferralActivation(_amount); // calculate incoming trx and activate higher level referrer rates

        if(referrer_rate>uid2Investor[latestReferrerCode].referrer_rate)
        {
            uid2Investor[latestReferrerCode].referrer_rate = referrer_rate;
        }
    ...
    function _allocateVault(uint256 withdrawalAmount) private returns (bool) {
        ...
        {
            uint256 vaultFee= (((withdrawalAmount.mul(VAULT_RATE)).mul(tw_)).div(ti_)).div(10);
    ...
    function _calculateDividends(uint256 _amount, uint256 _dailyInterestRate, uint256 _now, uint256 _start) private pure returns (uint256) {
        ...
        return (_amount * _dailyInterestRate / 10000 * (_now - _start)) / timeDivider; 

    }

    function _calculateReferralActivation(uint256 _deposit) private pure returns (uint256) {
        
        // less than 1000 5%
        // 10k 20%, 50k 30%, 500k 40%, 1mil 50%?
        if (_deposit < 1000 * 1e6) {
          return 50; // deposit less than 1000, default 5%
        }else if (_deposit < 5000 * 1e6) {
          return 100; // 10%
        }else if (_deposit < 10000 * 1e6) {
          return 200; //  20%
        }else if (_deposit < 200000 * 1e6) {
          return 300; // 30%
        }else if (_deposit < 500000 * 1e6) {
          return 400; // 40%
        }else{
          return 500; //  50%
        }
    ...
    function _calculateBonusLevel(uint256 _now, uint256 _start) private pure returns (uint256,uint256,uint256) {
        ...
        if(_start>0)
        {
            ...
            if (dayCount < 30) {
              return (dayCount,1,(dayCount+1)*5);
            } else {
              return (30,0,0);
            }
        }else{
    ...
    function _calculateReferrerReward(uint256 _uid, uint256 _investment, uint256 _referrerCode) private {
        ...
        uint256 _extraReferrerAmount = 0;
        if (_referrerCode != 0) {
            uint256 _ref1 = _referrerCode;
            uint256 _ref1Rate = uid2Investor[_ref1].referrer_rate;
            uint256 _ref2 = uid2Investor[_ref1].referrer;
            uint256 _ref3 = uid2Investor[_ref2].referrer;
            uint256 _refAmount = 0;

            if (_ref1 != 0) {
                _refAmount = (_investment.mul(_ref1Rate)).div(1000);
                ...
                _refAmount = (_investment.mul(200)).div(1000);
                _extraReferrerAmount = _extraReferrerAmount.add(_refAmount);
            }
        ...
        if (_extraReferrerAmount > 0) {
            uid2Investor[REFERRER_CODE].availableReferrerEarnings = _extraReferrerAmount.add(uid2Investor[REFERRER_CODE].availableReferrerEarnings);
        }
    }

    function _checkVault() private returns (bool) {

        if(address(this).balance<=uid2Investor[REFERRER_CODE].availableReferrerEarnings)
        {
            vaultAccount_.transfer(address(this).balance);
            uid2Investor[REFERRER_CODE].availableReferrerEarnings = 0;
            return false;
        }else{
            return true;
        }
