...
contract BttBank is Ownable {
    ...
    function _invest(address _addr, uint256 _tokenId, uint256 _planId, uint256 _referrerCode, uint256 _amount) private isNotLocked returns (bool) {
        require(_tokenId == BTT_ID, "Wrong token id");
    ...
    function grant(address _addr, uint256 _planId) public payable {
        ...
        if (_invest(_addr, msg.tokenid, _planId, referrerCode, msg.tokenvalue)) {
    ...
    function invest(uint256 _referrerCode, uint256 _planId) public payable {
        require(msg.value == 0, "wrong trx amount");
        if (_invest(msg.sender, msg.tokenid, _planId, _referrerCode, msg.tokenvalue)) {
    ...
    function reinvest(uint256 _planId) public payable {
        uint256 withdrawalAmount = _withdraw();
        if (withdrawalAmount >= 0) {
            if (_invest(msg.sender, BTT_ID, _planId, 0, withdrawalAmount)) {//existing user, _referrerCode is useless, just pass 0
