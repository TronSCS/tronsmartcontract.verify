...
contract FaithOfTron {
    ...
    string public name = "Faith of Tron";
    ...
    uint256 public whaleBalanceLimit = 600000000000; //600000trx
   ...
   constructor (address _marketing) public {
    ...
    function buy(address _referredBy) antiEarlyWhale isControlled public payable  returns (uint256) {
    ...
    function entryFee() public view returns (uint8){
      ...
      if (volume<=1000000000000){ //1M trx
        return 22;
      }
      if (volume<=2000000000000){ //2M trx
        return 21;
      }
      if (volume<=3000000000000){ //3M trx
        return 20;
      }
      if (volume<=4000000000000){ //4M trx
        return 19;
      }
      if (volume<=5000000000000){ //5M trx
        return 18;
      }
      return 17;
    }
     ...
     * 30% from entryFee is going to JackPot 
     * The last investor (with 5000+ TRX) will receive the jackpot in 12 hours 
     */	
	function calculateJackPot(uint256 _incomingTron, address _customerAddress) internal {
		...
		if (timer > 0 && jackPotPretender_ != address (0) && jackPot_ > 0) {
			...
			jackPotPretender_.transfer(jackPot_);
			jackPot_ = 0;
			jackPotStartTime_ = now;
		...
		uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingTron, entryFee()), 100);
		jackPot_ += SafeMath.div(SafeMath.mul(_undividedDividends, 30), 100);
		
		if (_incomingTron >= 5000000000) { 
