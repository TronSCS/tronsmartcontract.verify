...
contract TronJedies {
    ...
    event Collect(address indexed from, uint256 value);
    ...
    function buy(address _referredBy, uint256 _type, uint256 _number) public {
        ...
        dividendPerToken += paymentCoins.mul(3).div(100).div(totalGlorySupply);
        ...
        if (_referredBy != address(0) && _referredBy != msg.sender && players[_referredBy].time > 0) {
            players[_referredBy].coinsForSale = players[_referredBy].coinsForSale.add(paymentCoins.mul(30).div(1000));
            players[owner_].coinsForSale = players[owner_].coinsForSale.add(paymentCoins.mul(70).div(1000));
        } else {
