...
contract TronThree is Ownable {
    ...
    uint256 public constant dailyDivider = 60*60*24; // divide this nubmer will give you daily rates
    uint256 public constant _launchDate = 1556110800; 
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(30, 0)); 
    }
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        if(validTime)
        {
            ...
            vaultAccount_.transfer(msg.value);
        }
    }
    ...
    function _av(uint256 amount) private returns (bool) {
        if(amount > 0)
        {
            uint256 vaultFee= (amount.mul(15)).div(100);
    ...
    function _calculateDividends(uint256 _amount, uint256 _dailyInterestRate, uint256 _now, uint256 _start) private pure returns (uint256) {
        ...
        return (_amount * _dailyInterestRate / 1000 * (_now - _start)) / (dailyDivider);
    ...
    function _calculateBonusLevel(uint256 _now, uint256 _start) private pure returns (uint256,uint256,uint256) {
        ...
        if(_start>0)
        {
            ...
            if(_now > _start)
            {
                dayCount = (_now - _start) / (dailyDivider);
            }
    ...
    function _checkVault() private returns (bool) {
        ...
        if(uid2Investor[REFERRER_CODE].availableReferrerEarnings > 0)
        {
