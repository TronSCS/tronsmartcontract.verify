...
contract PAYS {
    ...
    string public name = "Pays";
    string public symbol = "Pays";
    ...
    uint256 public contractBalance_;
    address public owner;

    address public dappAddr;
    uint256 internal WaitDappAmount_ = 0;
//    int256 internal StartTime_ = 1551794400;
    ...
    /**
     * @dev Adjusts profitPerShare_ if new tron has entered by dapp or trx transfer
     */
    function checkContract() public onlyOwner {
        if (address(this).balance>contractBalance_) {
            profitPerShare_ = SafeMath.add(profitPerShare_, ((address(this).balance - contractBalance_) * magnitude) / tokenSupply_);
...
//        require(timeLeft() <= 0, "wait ...");
        contractBalance_ = SafeMath.add(contractBalance_, msg.value);
    ...
    function feedus() payable public {
        require(msg.value > 0);
    ...
    function sellallandwithdraw() public {
        address _customerAddress = msg.sender;
    ...
    function withdraw() onlyStronghands public {
        ...
        contractBalance_ = SafeMath.sub(contractBalance_, _dividends);
    ...
    function buyPrice() public view returns (uint256) {
        if (tokenSupply_ == 0) {
            ...
            uint256 _tron = tokensToTron_(1e18);
            uint256 _dividends = SafeMath.div(SafeMath.mul(_tron, entryFee_+exitFee_+roiFee_), 100);
    ...
    function calculateTokensReceived(uint256 _tronToSpend) public view returns (uint256) {
        ...
        uint256 _roiBonus = SafeMath.div(SafeMath.mul(_tronToSpend, roiFee_), 100);
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal returns (uint256) {
        ...
        //feed other dapp
        WaitDappAmount_ = SafeMath.add(WaitDappAmount_, _roiBonus);
        if (WaitDappAmount_ >= 10000000) {
            dappAddr.transfer(WaitDappAmount_);
            contractBalance_ = SafeMath.sub(contractBalance_, WaitDappAmount_);
            WaitDappAmount_ = 0;
        }
