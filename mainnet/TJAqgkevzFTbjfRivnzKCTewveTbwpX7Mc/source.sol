...
contract TronDay {
    ...
    function buy(uint _type, uint _number) public {
        ...
        players[owner].coinsForSale = players[owner].coinsForSale.add( paymentCoins.mul(50).div(1000) );
        players[manager].coinsForSale = players[manager].coinsForSale.add( paymentCoins.mul(50).div(1000) );
    ...
    function playerInfo(address adrs) public view returns (uint[TYPES_FACTORIES], uint, uint, uint){
        uint a = players[adrs].coinsForBuy;
        uint b = players[adrs].coinsForSale;
        uint c = players[adrs].time;
        return (players[adrs].factories, a, b, c);
    }
    
    function cntrctInfo() public view returns (uint, uint, uint, uint){
        uint contractBalance = address(this).balance;
        uint totalInvested = contractBalance.add(totalPayout);
        return(totalPlayers, totalFactories, totalPayout, totalInvested);
    }
