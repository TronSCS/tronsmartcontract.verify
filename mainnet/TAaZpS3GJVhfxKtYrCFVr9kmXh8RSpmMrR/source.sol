...
contract BabyWhales {
    ...
    uint256 public maxEarlyStake = 15000000000; //15000trx
    ...
    /// @dev starting
    uint256 public startTime = 1554246000; //  02 apr 2019 @ 11:00pm (UTC)	
   ...
   constructor (address _marketing) public {
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy , address _customerAddress) internal returns (uint256) {
		...
		uint256 _marketing = SafeMath.div(SafeMath.mul(_incomingTron, 2), 100); //2%
	...
	function calculateJackPot(uint256 _incomingTron, address _customerAddress) internal {
		uint256 timer = SafeMath.div(SafeMath.sub(now, jackPotStartTime_), 3 minutes); 
