...
contract TronVillageViewer {
    ...
    TronVillage tronVillage;

    constructor(address _addr) public {
        tronVillage = TronVillage(_addr);
    }

    function statsOf(address _addr) public view returns(
        uint treasury,
        uint spare,
        uint hourlyProfit,
        uint bakery,
        ...
        uint[TYPES_FACTORIES] memory factories = tronVillage.factoriesOf(_addr);

        bakery          = factories[0];
        teashop         = factories[1];
        confectionery   = factories[2];
        beeYard         = factories[3];
        brewhouse       = factories[4];
        perfumery       = factories[5];
        ...
        uint time;
        (treasury, spare, time) = tronVillage.players(_addr);
