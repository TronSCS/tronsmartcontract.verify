...
contract Market is MarketInterface, Pausable, TokenRecipientInterface, TokenFallback
{
        ...
        public whenNotPaused payable
    {
        ...
        Auction memory auction = Auction(
            ...
            uint40(now),
            uint128(msg.value),
            allowTokens ? allTokens : new address[](0)
        );
    ...
    {
        require(_areTokensAllowed(_allowedTokens));
        require(_isOwner(_seller, _cutieId));
        _escrow(_seller, _cutieId);
    ...
    function _areTokensAllowed(address[] _tokens) internal view returns (bool)
    {
    ...
    function getPriceInToken(ERC20Interface _tokenContract, uint128 priceWei)
        public
        view
        returns (uint128)
    {
    ...
    function tokenFallback(address _sender, uint _value, bytes _extraData) external whenNotPaused
    {
        ...
        require(_isTokensAllowed(auction.allowedTokens, tokenContractAddress)); // buy for token is allowed
    ...
    function _bidWithToken(address _tokenContract, uint40 _cutieId, address _sender) internal
    {
        ...
        require(_isTokensAllowed(auction.allowedTokens, _tokenContract)); // buy for token is allowed
    ...
    function _isTokensAllowed(address[] _allowedTokens, address _currentToken)
        internal
        pure
        returns (bool)
    {
...
/*
    ...
    function withdrawTokenFromBalance(ERC20Interface _tokenContract, address _withdrawToAddress) external
    {
        address coreAddress = address(coreContract);
    ...
    function addToken(ERC20Interface _tokenContract, PriceOracleInterface _priceOracle) external onlyOwner
    {
    ...
    function removeToken(ERC20Interface _tokenContract) external onlyOwner
    {
