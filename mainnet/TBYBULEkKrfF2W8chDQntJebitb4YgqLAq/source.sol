...
contract GardenOfTron{
	...
	uint256 minimumInvest = 4000000; //4 trx
    ...
    function sellVegetables() public isInitialized {
    ...
    function buyField() external payable isInitialized {
	...
	function reInvest() external isInitialized {
    ...
    function getFreeField() external isInitialized {
		Farmer storage farmer = farmers[msg.sender];
		if (farmer.vegetableId == 0) {
			//Set random vegetale for a new farmer	
			rollFieldId();
    ...
    function initMarket(uint256 _newTradeBalance) external payable onlyAdmin{
        require(!initialized);
        initialized = true;
		gameStarted = true;
