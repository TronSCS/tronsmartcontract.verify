...
/*
** This contract code is published only for verification purpose.
** You have no permission to copy it. If you want to use this contract contact
** the author: https://github.com/tronworlds
** This simple contract don't need Safe Math library. We always explicitly check values
** before every subtraction even if this check is redundant.
** And we add, divide and multiply only constants and trusted values (msg.value and now). 
** It's impossible to owerflow uint256 values in these conditions even 
** if somebody sends all TRX in existance to this contract.
*/
contract HonestRoi {
    ...
    function deposit() external payable startTimeCheck() {
        require(msg.value >= MIN_DEPOSIT, "Minimum is 10 TRX");
    ...
    function reinvestAll() external {
        User storage user = users[msg.sender];
        _userCheck(user);
        require(user.checkedProfit >= MIN_DEPOSIT, "Minimum is 10 TRX");
    ...
    function _userCheck(User storage user) internal {
        require(user.checkTime > 0, "Deposit some fund first");
