...
contract BlockchainCutiesPresale is Pausable
{
    ...
    function bid(uint32 cutieKind) public payable whenNotPaused
    {
        ...
        p.leftCount--;
        p.price = uint128(uint256(p.price)*p.priceMul / 1000000+ p.priceAdd);
    }
    ...
    function setParty(address _party) public onlyOwner
    {
    ...
    function withdrawEthFromBalance() external
    {
        ...
        require(partyAddress != 0);

        uint256 total = address(this).balance;

        partyAddress.transfer(total);
    }
}
