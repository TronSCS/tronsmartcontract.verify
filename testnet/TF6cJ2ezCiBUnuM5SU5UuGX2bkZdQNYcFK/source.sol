...
contract Game3TRX {
    ...
    uint constant TYPES_GPU = 6;
    uint constant PERIOD = 60 minutes;

    uint[TYPES_GPU] prices = [3000, 11750, 44500, 155000, 470000, 950000];
    uint[TYPES_GPU] profit = [4, 16, 62, 220, 680, 1400];

    uint public totalPlayers;
    uint public totalgpu;
    ...
    function buy(uint _type, uint _number) public {
        require(_type < TYPES_GPU && _number > 0);
        collect(msg.sender);
        ...
        player.gpu[_type] = player.gpu[_type].add(_number);
    ...
    function collect(address _addr) internal {
        ...
        if (hoursPassed > 0) {
            uint hourlyProfit;
            for (uint i = 0; i < TYPES_GPU; i++) {
                hourlyProfit = hourlyProfit.add( player.gpu[i].mul(profit[i]) );
            }
    ...
    function gpuOf(address _addr) public view returns (uint[TYPES_GPU]) {
        return players[_addr].gpu;
    }
